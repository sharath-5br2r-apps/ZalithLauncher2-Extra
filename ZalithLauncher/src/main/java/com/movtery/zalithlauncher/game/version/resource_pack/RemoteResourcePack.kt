...
private const val TAG = "RemoteResourcePack"

class RemoteResourcePack(
    val info: ResourcePackInfo
) {
    var isLoading by mutableStateOf(false)
        private set

    var projectInfo: ResourcePackProjectInfo? by mutableStateOf(null)
        private set
    ...
    suspend fun load(loadFromCache: Boolean) {
        ...
        try {
            withContext(Dispatchers.IO) {
                val file = info.file
                val projectCache = resourcePackProjectCache()

                runCatching {
                    val sha1 = calculateFileSha1(file)

                    val cachedProject = if (loadFromCache) {
                        projectCache.decodeParcelable(sha1, ResourcePackProjectInfo::class.java)
                    } else null
                    ...
                    if (loadFromCache && cachedProject != null) {
                        ...
                        if (projectRef != null) {
                            ...
                            val newProjectInfo = ResourcePackProjectInfo(
                    ...
                    Logger.warning(TAG, "Failed to load project info for resource pack: ${file.name}", e)
                }
            }
        } finally {
            isLoading = false
